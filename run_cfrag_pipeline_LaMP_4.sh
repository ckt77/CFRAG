...
# 相似用戶聚合模組配置
USE_SIMILAR_USER_AGGREGATION=true  # 是否啟用相似用戶聚合模組
SIMILAR_USER_TOPK=3                # 檢索的 top-k 相似用戶數量
AGGREGATION_METHOD="weighted_sum"    # 聚合方法: "attention" 或 "weighted_sum"
...
get_latest_retriever_checkpoint() {
    ...
    # 返回格式: 模型名/時間戳（例如: bge-base-en-v1.5/20251215-122556）
...
get_latest_reranker_checkpoint() {
    ...
    # 返回格式: 模型名/時間戳（例如: bge-reranker-base/20251215-123347）
    local model_name=$(basename "$model_base_dir")
    local timestamp=$(basename "$latest_timestamp_dir")
    echo "${model_name}/${timestamp}"
}
...
print_info "開始 CFRAG LaMP_4 完整訓練流程"
...
if [ "$SKIP_STEP1" = false ]; then
    ...
    # python data/preprocess_profile.py --data_phase train --task "$TASK"
    # python data/preprocess_profile.py --data_phase dev --task "$TASK"
    
    # # 1.2 獲取用戶集合
    # print_info "1.2 獲取用戶集合..."
    # python user_emb/get_user_set.py --task "$TASK" --source "$SOURCE"
    
    # # 1.3 獲取語料庫嵌入
    # print_info "1.3 獲取語料庫嵌入..."
    # CUDA_VISIBLE_DEVICES=$CUDA_DEVICE python user_emb/get_corpus_emb.py --task "$TASK" --source "$SOURCE" --stage "$DATA_SPLIT_DEV"
    
    # 1.4 訓練用戶嵌入
    print_info "1.4 訓練用戶嵌入..."
    cd user_emb/train_user_emb
    
    # 構建訓練命令參數
    TRAIN_ARGS="--task $TASK --stage $DATA_SPLIT_DEV --source $SOURCE"
...
if [ "$SKIP_STEP2" = false ]; then
    ...
    # 查找最新的訓練點文件
    POINT_FILE=$(find "../../${MODEL_NAME}_outputs/${TASK}/${DATA_SPLIT_TRAIN}/${SOURCE}" -name "predictions_*.json" -path "*/point_*" -type f 2>/dev/null | xargs ls -t 2>/dev/null | head -1)
    if [ -z "$POINT_FILE" ] || [ ! -f "$POINT_FILE" ]; then
        print_error "未找到訓練點文件"
        print_error "搜索目錄: ../../${MODEL_NAME}_outputs/${TASK}/${DATA_SPLIT_TRAIN}/${SOURCE}"
        exit 1
    fi
    
    print_info "使用訓練點文件: $POINT_FILE"
    
    # 構建完整的用戶嵌入路徑（相對於 rank_tune/retriever 目錄）
    USER_EMB_FULL_PATH="../../data/${TASK}/${DATA_SPLIT_DEV}/${SOURCE}/user_emb/${USER_EMB_FILE}"
    USER_VOCAB_PATH="../../data/${TASK}/${DATA_SPLIT_DEV}/${SOURCE}"
    
    print_info "使用用戶嵌入: $USER_EMB_FULL_PATH"
    print_info "使用用戶詞彙: $USER_VOCAB_PATH"
    
    NCCL_P2P_DISABLE=1 NCCL_IB_DISABLE=1 CUDA_VISIBLE_DEVICES=$CUDA_DEVICE python run.py \
        --train_data "$POINT_FILE" \
        --user_vocab_path "$USER_VOCAB_PATH" \
        --user_emb_path "$USER_EMB_FULL_PATH" \
        --output_dir "../../${MODEL_NAME}_outputs/${TASK}/${DATA_SPLIT_TRAIN}/${SOURCE}/" \
        --main_metric rouge-1 \
        --do_train
    cd "$PROJECT_ROOT"
...
if [ "$SKIP_STEP4" = false ]; then
    ...
    # 例如: Qwen2-7B-Instruct_outputs/LaMP_4/dev/recency/bge-base-en-v1.5_5/retrieval/20251215-122556_user-6_20251215-120616.json
    ...
    # 例如: .../dev/recency/bge-base-en-v1.5_5/bge-reranker-base/20251215-123347_rerank_5/{file_name}.json
    # 其中 result_name = bge-reranker-base/20251215-123347_rerank_5 (包含模型名稱和時間戳)
    
    RERANK_DIR=$(dirname "$RERANK_FILE_FULL_PATH")  # .../bge-reranker-base/20251215-123347_rerank_5
    RESULT_NAME_WITH_MODEL=$(basename "$RERANK_DIR")  # 20251215-123347_rerank_5
    ...
    RESULT_NAME="${MODEL_NAME_PART}/${RESULT_NAME_WITH_MODEL}"  # bge-reranker-base/20251215-123347_rerank_5
